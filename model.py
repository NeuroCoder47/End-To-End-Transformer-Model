...
class DecoderLayer(nn.Module):
    def __init__(self, d_model, num_heads,d_ff, dropout):
        ...
        self.self_attn = MultiHeeadAttention(d_model, num_heads)
        self.cross_attn = MultiHeeadAttention(d_model, num_heads)
        self.feed_forward = PositionWiseFeedForward(d_model, d_ff)
        self.norm1 = nn.LayerNorm(d_model)
        self.norm2 = nn.LayerNorm(d_model)
        self.norm3 = nn.LayerNorm(d_model)
        self.dropout = nn.Dropout(dropout)
    def forward(self,x,enc_opt,src_mask, Tgt_mask):
         attn_output = self.self_attn(x,x,x,Tgt_mask)
         x =self.norm1(x+self.dropout(attn_output))
         attn_output = self.cross_attn(x,enc_opt,enc_opt,src_mask)
